...
const STYLE = `
/* The HUD face. Seven families are loaded and two are chosen by the variables below, so
   comparing a pairing is a one-line change rather than a hunt through the stylesheet:

     Alien Cybernetics  ON TEST - a single unicase weight, currently driving BOTH roles at
                        once so it can be judged as one look before any split is decided.
                        Rounded sci-fi letterforms; lowercase renders identical to uppercase
                        (confirmed by rendering the glyphs). Unlike Digital Tech, punctuation
                        is real here - period, comma, apostrophe, hyphen and the rest all have
                        actual outlines, checked the same way Digital Tech's were found empty.
     Digital Tech       an earlier test - a single unicase weight. Its punctuation glyphs are
                        present in the character map but every one has a ZERO outline (period,
                        comma, apostrophe, hyphen, dashes, quotes, degree sign, ellipsis all
                        confirmed empty), so real copy loses its punctuation silently. Kept
                        loaded for comparison, not recommended as-is.
     Space Future Bold  an earlier DISPLAY face - every button (.hud-pill, the tabs and
                        controls inside a card, the close orb) and the hint pill.
     Space Future Reg   an earlier TEXT face - star-info and lore panel body copy. Real
   ...
   of the faces on test here have yet. Until that exists, the in-world "3D View" / "Lore"
   buttons stay in Origin Tech regardless of what is on test in this stylesheet. */
@font-face {
  font-family: 'Alien Cybernetics';
  src: url('assets/fonts/AlienCybernetics-Regular.woff2') format('woff2');
  font-display: swap;
}
@font-face {
...
#hud {
  /* The two variables below are the whole swap: point them at a different family and every
     ...
     Both point at Alien Cybernetics right now, on test - see the comment above. */
  --hud-display: 'Alien Cybernetics', Arial, sans-serif;
  --hud-text: 'Alien Cybernetics', Arial, sans-serif;
  --u: 1vmin;
  --vh: 1vh;
  --accent: #4287f5;
